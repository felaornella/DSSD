...
def generar_qr(id):
    ...
    # Get qr from the api 
    url = "localhost:5000/sociedad/"+soc.hash  # Url de la sociedad que se va a mostrar
    # Find if the qr exists in app/static/qr/ 
    qr_path = Path("app/static/temp/qr/qr_"+soc.hash+'.png')
    if not qr_path.exists():
        print("no existe se crea y se guarda en drive")
        img = qrcode.make(url)
        img.save('app/static/temp/qr/qr_'+soc.hash+'.png')
        GD.subir_archivo("app/static/temp/qr/qr_"+soc.hash+".png",GD.folder_qr)
    else:
        print("existe") 
...
def obtener_qr(hash):
    ...
    qr_path = Path("app/static/temp/qr/qr_"+hash+'.png')
    if not qr_path.exists():
        print("no existe - lo busca en drive")
        GD.bajar_acrchivo_por_nombre("qr_"+hash+'.png',"app/static/temp/qr/")
        # Return error "file not found" 
        if not qr_path.exists():
            print("no existe ni en drive")
            return jsonify({'msg':'File not found'}),404,{'ContentType':"application/json"}
    
    return send_file('static\\temp\\qr\\qr_'+hash+'.png', mimetype='image/png')
